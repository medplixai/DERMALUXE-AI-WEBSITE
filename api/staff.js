...
module.exports = async (req, res) => {
  ...
  if (a === "prices-set" || a === "cycles-set" || a === "boost-set" || a === "goals-set" || a === "opener-set") {
    if (req.method !== "POST") return json(res, 405, { error: "POST" });
    ...
    const out = a === "prices-set" ? await require("./_prices.js").save(cfg, b, me.name)
      ...
      : a === "opener-set" ? await require("./_abtest.js").save(cfg, b, me.name)
    ...
    await audit(cfg, me, a === "prices-set" ? `Price policy: ${out.prices.mode}${out.prices.consult ? " · consult ₹" + out.prices.consult : ""}`
      ...
      : a === "opener-set" ? `Ad opener test: ${out.opener.on ? "on" : "off"} — "${out.opener.a.text.slice(0, 40)}" vs "${out.opener.b.text.slice(0, 40)}"`
  ...
  if (a === "panel") {
    ...
    const [agentRules, team2, prices, cycles, boost, boosts, goals, goalState, opener] = await Promise.all([
      ...
      require("./_abtest.js").load(cfg).catch(() => null),
    ]);
    return json(res, 200, {
      ok: true, roles, capList: CAPS, capTe: CAP_TE, capGroups: CAP_GROUPS, people, health,
      agentRules, callTeam: team2, prices, cycles, goals, goalState, opener,
